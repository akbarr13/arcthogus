...
export default function StorePage() {
  ...
  async function submitOrder() {
    ...
    try {
      ...
      paymentUrlRef.current = result.payment_url || ''
      closeModal('orderOverlay')

      // Tampilkan success modal dengan link pembayaran
  ...
  function lanjutBayar() {
    ...
    if (!url) return
    const btn = document.getElementById('btnLanjutBayar') as HTMLButtonElement
    if (btn) { btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Mengalihkan...' }
    window.location.href = url
  }

  function selesai() {
    closeModal('successOverlay')
    selectedSizeRef.current = null; qtyRef.current = 1; currentOrderIdRef.current = ''; paymentUrlRef.current = ''
  ...
  async function cekStatus() {
    ...
    try {
      ...
      const payBtn = row.payment_url && s === 'pending'
        ? `<div style="margin-top:12px"><a href="${esc(row.payment_url)}" class="btn-sub" style="display:inline-block;text-decoration:none;padding:10px 20px;font-size:.85rem"><i class="fa fa-qrcode"></i> Bayar Sekarang</a></div>`
        : ''
  ...
  if (typeof window !== 'undefined') {
    ...
    (window as unknown as Record<string, unknown>).lanjutBayar = lanjutBayar;
    ...
    <>
      ...
      <div className="overlay" id="successOverlay">
        <div className="mbox" style={{ textAlign: 'center' }}>
          ...
          <h2>Order <span className="text-accent">Berhasil!</span></h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '20px', fontSize: '.9rem', lineHeight: 1.6 }}>
            Ordermu sudah dibuat. Simpan Order ID di bawah, lalu lanjut ke halaman pembayaran untuk scan QRIS.
          </p>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '.78rem', margin: '12px 0 20px' }}>
            Gunakan Order ID ini untuk tracking status pesanan.
          </p>
          <button id="btnLanjutBayar" className="btn-sub full" onClick={lanjutBayar} style={{ marginBottom: '10px' }}>
            <i className="fa fa-qrcode" /> Lanjut ke Pembayaran
          </button>
          <button
            onClick={selesai}
            style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: '.82rem', cursor: 'pointer', textDecoration: 'underline' }}
          >
            Bayar Nanti
          </button>
        </div>
